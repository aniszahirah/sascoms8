				  ...
				  <tr>
					<td class="px-6 py-4 whitespace-nowrap">
					  <div class="flex items-center">
						<div class="ml-4">
						  <div class="text-sm font-medium text-gray-900">{{ $clinic->id }}</div>
						</div>
					  </div>
					</td>
					<td class="px-6 py-4 whitespace-nowrap">
					  <span class="text-sm font-medium text-gray-900">{{ $clinic->issue }}</span>
					</td>
					<td class="px-6 py-1 whitespace-nowrap">
						<span class="text-sm font-medium text-gray-900">{{ $clinic->followup }}</span>
					  </td>
					...
					<td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
					  <button class="bg-indigo-200 hover:bg-indigo-300 text-indigo-600 hover:text-indigo-900 font-bold py-1 px-2 rounded inline-flex items-center">
						  ...
						  <a href="{{ action('App\Http\Controllers\Form\ClinicController@view', $clinic->id) }}" class="px-2 items-center text-indigo-600 hover:text-indigo-900">View</a>
					  </button>
					  <button class="bg-green-200 hover:bg-green-300 text-green-800 hover:text-green-900 font-bold py-1 px-2 rounded inline-flex items-center">
						  <svg style="width:24px;height:24px" viewBox="0 0 24 24">
							...
							<a href ="{{ action('App\Http\Controllers\Form\ClinicController@pdf', $clinic->id) }}" class="btn btn-danger" >Download PDF</a>
						</button>
					  </td>
					</tr>
